...
@Controller()
export class JoloQuoteSubmitController {
  ...
  constructor(
    ...
    this.logger = logger.child({ context: 'JoloQuoteSubmitController' })
  ...
  private get submitted$() {
    return this.parserSvc.getOnParseObservable<IJoloSubmitCommandParams>().pipe(
      ...
      map(({ message, params }) => {
        ...
        return {
          content,
          message,
        } as IJoloSubmitHandlerParams
      })
    )
  }

  private async fetchSubmissionRequirements({
    message,
  }: IJoloSubmitHandlerParams) {
  ...
  private async submitToCoreMicroservice(
    { content, message }: IJoloSubmitHandlerParams,
   ...
   */
  async handler(params: IJoloSubmitHandlerParams): Promise<void> {
...
interface IJoloSubmitHandlerParams {
  content: string
  message: Message
}

interface IJoloSubmitCommandParams {
  content: string
}
